...
class MDCData(scapy.Packet):
    ...
    fields_desc = [
        ...
        scapy.ByteEnumField(
            "code",
            5,
            {1: "REQUEST", 2: "PTCH_REQ", 3: "DATA_FNSD", 4: "PTCH_DATA", 5: "DATA"},
        ),
        scapy.XByteField("appID", 2),
        scapy.XByteField("dataID", 0),
        scapy.XByteField("sn", 0),
        scapy.XByteField("dataSize", 9),
    ]
...
if os.path.exists("/tmp/mdc_dp_p.sock"):
    ...
    while sending_pk_sn < 10:
        try:
            ...
            print(
                "Sendiing unlabeled_data_mdc_header: ",
    ...
    while True:
        try:
            ...
            if len(data) > 0:
                ...
                data_ip_header = data[
                    len(bytes(data_eth)) : len(bytes(data_eth)) + len(bytes(data_ip))
                ]
                data_udp_header = data[
                    len(bytes(data_eth))
                    + len(bytes(data_ip)) : len(bytes(data_eth))
                    + len(bytes(data_ip))
                    + len(bytes(data_udp))
                ]
                unlabeled_data_mdc_header = data[
                    len(bytes(data_eth)) + len(bytes(data_ip)) + len(bytes(data_udp)) :
                ]
                print("unlabeled_data_mdc_header: ", unlabeled_data_mdc_header)

                print(
                    "unlabeled_data_mdc_header: ",
                    struct.unpack(">B", unlabeled_data_mdc_header[8:9]),
                    " ",
                    unlabeled_data_mdc_header[8:9],
                )
                print(
                    "unlabeled_data_mdc_header addr: ",
                    struct.unpack(">H", unlabeled_data_mdc_header[:2]),
                    " ",
                    unlabeled_data_mdc_header[:2],
                )
                print(
                    "unlabeled_data_mdc_header mode: ",
                    struct.unpack(">B", unlabeled_data_mdc_header[2:3]),
                    " ",
                    unlabeled_data_mdc_header[2:3],
                )
